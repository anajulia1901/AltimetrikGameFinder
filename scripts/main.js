...
function starting() {
  ...
  document.querySelector("#search-input").addEventListener("keyup", searchGame);
}
                ...
                <div class="photoContainer">
                    ...
                    <div class="bottom-info-card">
                        <div class="title-plataform-container">
                            <p class="game-title" >${allInfo.name}</p>
                            <div class="plataforms-icons">
                                ${cardConsols(allInfo)}
                            </div> 
                         </div>
                        <div class="date-gif">
                    ...
                    <div class="desription-hidden"></div>
                </div>
            </div>
        </div>
     </div> `;
    document.querySelector(".videogame-cards").innerHTML += card;
    globalId.push(allInfo.id);
  }
  descriptionText(globalId);
}
async function descriptionText(id) {
  const descriptionContainer = document.querySelectorAll(".desription-hidden");
  for (let i = 0; i < descriptionContainer.length; i++) {
    let informationDescriptionCard = await getInformationApi(id[i]);
    descriptionContainer[i].innerHTML +=
      informationDescriptionCard.description_raw;
  }
}
function cardConsols(cardInfo) {
  ...
  for (let i = 0; i < cardInfo.parent_platforms.length; i++) {
    if (i < 3) {
      ...
      consols += "<h3 class='title-plataform-container-dots dots'>...<h3>";
      return consols;
    }
  }
  return consols;
...
function gridToColumn() {
  ...
  document.querySelector(".grid-button").addEventListener("click", gridClick);
}
function columnClick() {
  oneColumn = true;
  document
    .querySelector(".videogame-cards")
    .classList.add("videogame-cards-column");
  let cardContainer = document.querySelectorAll(".one");
  let cardContainerImg = document.querySelectorAll(".photo1");
  let cardContainerTitle = document.querySelectorAll(
    ".title-plataform-container"
  );
  let cardContainerInfo = document.querySelectorAll(".info-container");
  let cardContainerLine1 = document.querySelectorAll(".line1");
  let cardContainerLine2 = document.querySelectorAll(".line2");

  for (let i = 0; i < cardContainer.length; i++) {
    document
      .querySelectorAll(".desription-hidden")
    [i].classList.add("description-show");
    cardContainer[i].classList.add("one1");
    cardContainerImg[i].classList.add("photo1-column");
    cardContainerTitle[i].classList.add("title-plataform-container-column");
    cardContainerInfo[i].classList.add("info-containerColumn");
    cardContainerLine1[i].classList.add("line-column1");
    cardContainerLine2[i].classList.add("line-column2");
  }
}
...
function gridClick() {
  ...
  document
    .querySelector(".videogame-cards")
    .classList.remove("videogame-cards-column");
  let cardContainer = document.querySelectorAll(".one");
  let cardContainerImg = document.querySelectorAll(".photo1");
  let cardContainerTitle = document.querySelectorAll(
    ".title-plataform-container"
  ...
  for (let i = 0; i < cardContainer.length; i++) {
    document
      .querySelectorAll(".desription-hidden")
      [i].classList.remove("description-show");
    cardContainer[i].classList.remove("one1");
    cardContainerImg[i].classList.remove("photo1-column");
    cardContainerTitle[i].classList.remove("title-plataform-container-column");
    cardContainerInfo[i].classList.remove("info-containerColumn");
    cardContainerLine1[i].classList.remove("line-column1");
    cardContainerLine2[i].classList.remove("line-column2");
...
async function functionModal(id) {
  console.log(id);
  let modal = document.querySelector(`.modal-selected`);
  ...
  document.querySelector(".modal-icons").innerHTML = modalConsols(newInfoApi);
...
function modalConsols(newInfoApi) {
  let consols = "";
  let thisConsol = "";
  for (let i = 0; i < newInfoApi.parent_platforms.length; i++) {
    switch (newInfoApi.parent_platforms[i].platform.slug) {
      case "ios":
        thisConsol = `<svg class="svg-space svgModal" width="15" height="16" version="1.1" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    viewBox="0 0 22.773 22.773" xml:space="preserve" fill="white">
...
async function getScreenshot(slug) {
  const fetchData = await fetch(
    `https://api.rawg.io/api/games/${slug}/screenshots?key=a6b177c4bf7e429c91a64c0f6d30398c`
  );
  ...
  document
    .querySelector(".modal-video-style")
    .setAttribute("src", `${screenshotsModal[0]}`);
...
async function fetchSearch(input) {
  let fetchSearch = await fetch(
    `https://api.rawg.io/api/games?key=a6b177c4bf7e429c91a64c0f6d30398c&search=${input}`
  );
  let inputData = await fetchSearch.json();
  document.querySelector("#title1-cards").innerHTML = "Search results";
  document.querySelector(".title2-cards").innerHTML = `"${input}" results`;
  document.querySelector(".videogame-cards").innerHTML = "";
  page = inputData.next;
        ...
        <div class="one cardGrid" onclick=functionModal(${allInfo.id})>
            <div class="photoContainer">
                ...
                <div class="bottom-info-card">
                    <div class="title-plataform-container">
                        <p class="game-title" >${allInfo.name}</p>
                        <div class="plataforms-icons"><svg width="60" height="13" viewBox="0 0 60 13" fill="none" xmlns="http://www.w3.org/2000/svg">
...
async function infiniteScroll(pageScroll) {
                  ...
                  <div class="photoContainer">
                      ...
                      <div class="bottom-info-card">
                          ...
                          <div class="info-container">
                              ...
                              <div class="number-gif">
                                  <p class="game-number">#${counter + i + 1}</p>
                                  <button class='card-interact-button' id="main-card">
                                  <div class="gift"><span class="plus">+</span><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      <path fill-rule="evenodd" clip-rule="evenodd" d="M3 2.5C3 1.11929 4.11929 0 5.5 0C6.88071 0 8 1.11929 8 2.5C8 1.11929 9.11929 0 10.5 0C11.8807 0 13 1.11929 13 2.5V2.506C13 2.576 13 2.776 12.962 3H15C15.5523 3 16 3.44772 16 4V5C16 5.55228 15.5523 6 15 6H1C0.447715 6 0 5.55228 0 5V4C0 3.44772 0.447715 3 1 3H3.038C3.01159 2.83668 2.99888 2.67144 3 2.506V2.5ZM4.068 3H7V2.5C7 1.9641 6.7141 1.46891 6.25 1.20096C5.7859 0.933013 5.2141 0.933013 4.75 1.20096C4.2859 1.46891 4 1.9641 4 2.5C4 2.585 4.002 2.774 4.045 2.93C4.05101 2.95385 4.05869 2.97724 4.068 3ZM11.932 3H9V2.5C9 1.67157 9.67157 1 10.5 1C11.3284 1 12 1.67157 12 2.5C12 2.585 11.998 2.774 11.955 2.93C11.9489 2.95381 11.9412 2.9772 11.932 3ZM15 7V14.5C15 15.3284 14.3284 16 13.5 16H9V7H15ZM1 14.5C1 15.3284 1.67157 16 2.5 16H7V7H1V14.5Z" fill="white"/>
                                      </svg>
                      ...
                      <div class="desription-hidden description-show"></div>
                  </div>
              </div>
          </div>
       </div> `;
  ...
  for (let i = 0; i < fetchPageInfo.results.length; i++) {
    ...
    let card = `<div class= "videogames">
        <div class="videogame-cards">
            <div class="one one1" onclick=functionModal(${allInfo.id})>
                <div class="photoContainer">
                    <img class="photo1 photo1-column" id="photo" src= '${
                      allInfo.background_image
                    }' >
                    <div class="bottom-info-card">
                        <div class="title-plataform-container title-plataform-container-column">
                            <p class="game-title" >${allInfo.name}</p>
                            <div class="plataforms-icons"><svg width="60" height="13" viewBox="0 0 60 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M60 5.95833H52.9583V0.998704L60 0V5.95833ZM52.4167 1.08333V5.95833H47V1.80612L52.4167 1.08333ZM52.4167 6.5H47V11.1145L52.4167 11.9167V6.5ZM52.9583 11.912V6.5H60V13L52.9583 11.912Z" fill="white"/>
                                <path d="M6.5 0.149439L6.5 12.0297L9.07955 12.882L9.07955 2.9205C9.07955 2.4511 9.28024 2.13944 9.60212 2.24662C10.023 2.36835 10.1048 2.80075 10.1048 3.26493L10.1048 7.24338C11.7104 8.05381 12.9745 7.24296 12.9745 5.10468C12.9745 2.91966 12.2334 1.94626 10.0527 1.16365C9.19249 0.864976 7.59836 0.360979 6.5 0.149439Z" fill="white"/>
                        ...
                        <div class="date-gif">
                        <div class="info-container info-containerColumn">
                            <div class="text-container">
                                <p class="release text-grid">Release date</p>
                                <p class="date text-grid">${
                                  allInfo.released
                                }</p>
                                <div class="line1 line-column1"></div>
                            </div>
                                ...
                                <div class="text-container">
                                    ...
                                    <div class="line2 line-column2"></div>
                                </div>
                            </div>
                            <div class="number-gif">
                                <p class="game-number">#${counter + i + 1}</p>
