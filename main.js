...
function starting() {
    ...
    document.querySelector("#searchInput").addEventListener("keyup",searchGame);
}
...
function cardMultiplication(cardInformation){
    for(let i= 0 ; i<cardInformation.results.length  ; i++){  
...
async function functionModal(id){
    ...
    document.querySelector("#topRanking").textContent = modalTop;
    ...
    document.querySelector(".modal-date-ranking").textContent = modalReleaseDates;
    ...
    let age =  getAgerating (newInfoApi.esrb_rating);
    let developers= getDevelopers(newInfoApi.developers[0])
    let publishers= getPublishers(newInfoApi.publishers[0])
    let website= getWebsite(newInfoApi.website)
}
function getWebsite(website){
    if (website === null){
        return "error";
    }
    document.querySelector(".websiteinfo").textContent = website;
}
function getPublishers(publishers){
    if (publishers === null){
        return "error";
    }
    document.querySelector(".publisherinfo").textContent = publishers.name;
}
function getDevelopers(developers){
    if (developers === null){
        return "error";
    }
    document.querySelector(".develop").textContent = developers.name;
}
function getAgerating(age){
    if (age === null){
        return "error";
    }
    document.querySelector(".age").textContent = age.name;
}
...
function getPlatforms(platforms){
    if (platforms === null){
        ...
        for (let i=0 ; i<platforms.length ; i++){
            platformSelected += platforms[i].platform.name + ", "
        }
        document.querySelector(".platform").textContent = platformSelected;
    }
}
...
async function getScreenshot(slug){
const fetchData = await fetch (`https://api.rawg.io/api/games/${slug}/screenshots?key=a6b177c4bf7e429c91a64c0f6d30398c`)
let slugData = await fetchData.json();
let screenshotsModal = [];
for( let i=0; i<slugData.results.length ; i++){
    screenshotsModal.push(slugData.results[i].image);
}
document.querySelector(".modal-video-style").setAttribute("src",`${screenshotsModal[0]}`);
document.querySelector(".img1").setAttribute("src",`${screenshotsModal[1]}`);
document.querySelector(".img2").setAttribute("src",`${screenshotsModal[2]}`);
document.querySelector(".img3").setAttribute("src",`${screenshotsModal[3]}`);
document.querySelector(".img4").setAttribute("src",`${screenshotsModal[4]}`);
}
 function searchGame (){
...
let fetchSearch = await fetch(`https://api.rawg.io/api/games?key=a6b177c4bf7e429c91a64c0f6d30398c&search=${input}`)
let inputData= await fetchSearch.json();
document.querySelector("#title1Cards").innerHTML= "Search results";
document.querySelector(".title2Cards").innerHTML= `"${input}" results`;
document.querySelector(".videogameCards").innerHTML= "";
page=inputData.next;
